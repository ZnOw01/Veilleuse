...
class ScheduleTests(HelperModuleTests):
    ...
    def test_update_profile_clean_line_removal(self):
        ...
        reverted = vc._update_profile(updated, "06:30", 5950, identity_day=False)
        self.assertEqual(reverted, "profile {\n    time = 06:30\n    temperature = 5950\n}")

    def test_update_profile_preserves_comments_on_removed_lines(self):
        original = "profile {\n    time = 06:00\n    temperature = 6000 # daylight\n}"
        updated = vc._update_profile(original, "06:30", 6000, identity_day=True)
        self.assertIn("# daylight", updated)
        self.assertNotIn("temperature =", updated)
        self.assertIn("identity = true", updated)

    def test_update_schedule_preserves_crlf_and_markers(self):
        crlf_text = (
            "# Generated by Veilleuse\r\n"
        ...
        updated = vc._update_schedule_text(
            crlf_text,
            {"day_time": "07:00", "day_temp": 6000, "night_time": "22:00", "night_temp": 3000},
            natural_day=True,
        )
        self.assertIn("\r\n", updated)
        self.assertNotIn("# Veilleuse day\n", updated)
        self.assertIn("# Veilleuse day\r\n", updated)

    def test_schedule_set_rejects_symlink_config(self):
        real_target = self.xdg / "real_sunset.conf"
        real_target.write_text("profile {\n    time = 06:00\n    identity = true\n}\nprofile {\n    time = 15:30\n    temperature = 3500\n}\n")
        config = vc.config_path()
        config.parent.mkdir(parents=True, exist_ok=True)
        config.symlink_to(real_target)
        try:
            state = vc.schedule_set(
                {
                    "day_time": "07:00",
            ...
            self.assertFalse(state["available"])
            self.assertIn("Ruta insegura", state["error"])
        finally:
            if config.is_symlink():
                config.unlink()

    def test_nightlight_failure_sets_available_false(self):
        with patch.object(vc, "read_nightlight", return_value={"available": True, "error": None}):
            failed = vc._nightlight_failure("Write error")
            self.assertFalse(failed["available"])
            self.assertIn("Write error", failed["error"])
